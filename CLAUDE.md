...
- 2026-09-14: Two bug fixes — notification not clearing on check-in, 12h/24h time format
   - **B1 — notification not cleared on completion**: marking a task Done/Partial/Skipped from its notification action left the notification sitting in the shade. `Notifier` posts each start alert under notification ID `blockId.hashCode()`, but nothing ever called `cancel()` on it. Added `Notifier.cancelStartNotification(blockId)` and call it from both places a check-in can happen: `CheckInReceiver.onReceive` (the notification-action path — now injects `Notifier`) and `ScheduleViewModel.checkIn` (the in-app `CheckInSheet` path, tapping a block in List/Grid), so either route dismisses the notification. Verified the fix by code/ID-matching review rather than a live end-to-end wait (confirming it would need waiting real-time minutes for an actual alarm to fire) — the two IDs (`notify`'s and `cancel`'s) are the same `blockId.hashCode()` computation, and `NotificationManagerCompat.cancel()` on an ID with nothing showing is a safe no-op either way.
   - **B2 — grid was hardcoded to 12-hour, everywhere else was hardcoded to 24-hour**: turned out to be inconsistent in *both* directions before this fix — `ScheduleGridScreen`'s hour ruler had its own `formatHourLabel` always rendering "12 AM"/"1 PM" etc., while `ScheduleListScreen`'s `formatMinutes`, `ScheduleDialScreen`'s Now-ring, and both add/edit sheets' time chips all did `String.format("%02d:%02d", ...)` (always 24h) with the native `TimePickerDialog` also hardcoded to `is24HourView = true`. Per the suggestion to make it a user setting rather than a hard switch: added `AppSettings.use24HourClock: Boolean` (Room schema bumped v2 → v3, `fallbackToDestructiveMigration()` still in effect for this dev stage), a "24-hour time" toggle in Settings → Appearance with a live "2:30 PM" / "14:30" preview, and a single shared `util/TimeFormat.kt` (`minutesLabel`, `hourLabel`) replacing the duplicated formatting logic in `ScheduleListScreen`/`ScheduleDialScreen`/`ScheduleGridScreen`. The setting is read via `SettingsViewModel` (same Hilt-scoped instance across a nav destination, so no extra plumbing) and threaded into `AddOrCopySheet`, `EditBlockSheet`, and all three schedule views, including the `TimePickerDialog`'s own `is24HourView` param so the native picker matches the rest of the app.
   - Verified via `./gradlew testDebugUnitTest assembleDebug`, then live on-device: toggled 24-hour time on in Settings (preview correctly flipped "2:30 PM" → "14:30"), confirmed the Grid ruler updated from "6 AM…11 PM" to "06:00…23:00" immediately.

- 2026-09-14: Lock-In Mode (F1-F6) — first implementation
   - **No API exists for "lock the device except camera/calls"** — that's the real starting fact for this feature. Incoming/emergency calls already can't be blocked by any third-party app (Telecom stack overrides everything), so that part of F3 needed no code. Blocking *other* apps requires either an Accessibility Service or Usage Access + a system overlay; per discussion, built on **Usage Access + overlay** (lower Play Store review risk, the pattern most published focus/blocker apps use) rather than Accessibility Service.
   - **Trigger interpretation**: F1 says "completing this task triggers lock-in," which doesn't work temporally (can't lock in after finishing) — implemented as triggering **at the task's scheduled start time** instead (same moment the existing start-alert fires), consistent with F5 describing a pre-start countdown for task-linked sessions too. Flagged this interpretation to the user; not yet confirmed.
   - **Break policy** (`domain/lockin/BreakPolicy.kt`, pure/testable): tasks ≤2h get one continuous session, no break. Tasks >2h: break slot *k* becomes available once *active* (non-break) elapsed minutes reach *k*×120, **and** only after break *k-1* was actually used (sequential gating, not banked) — per explicit answers: 2-hour interval, fixed 20-minute length, "one at a time, next unlocks after using the previous."
   - **Camera/phone allowlist** (`domain/lockin/AllowlistResolver.kt`): resolves *this device's* actual default camera (`MediaStore.ACTION_IMAGE_CAPTURE` → `PackageManager.resolveActivity`) and default dialer (`TelecomManager.defaultDialerPackage`) at runtime rather than hardcoding OEM package names.
   - **Core service** (`data/lockin/LockInForegroundService.kt`): a foreground service (`foregroundServiceType="specialUse"`, with the required Play Console justification property) that (1) runs the F5 10-second cancelable countdown notification before anything engages, (2) on engaging, polls `UsageStatsManager.queryEvents` every 1.5s for the current foreground package (the standard non-Accessibility way to detect it) and shows/hides a full-screen block overlay via `WindowManager` when it's outside the allowlist, (3) tracks active-vs-break elapsed time for the break policy, (4) ends the session and logs it via `LockInRepository` when planned duration is reached. State is shared with the UI via a plain `LockInStateHolder` singleton `StateFlow` rather than a bound service — simpler for single-process state.
   - **Overlay** (`domain/lockin/LockInOverlayController.kt`) is built from plain `View`s, not Compose — a `WindowManager`-attached view needs its own `ViewTreeLifecycleOwner`/`SavedStateRegistryOwner` wiring to host Compose, which this simple block screen doesn't need. It shows title/task/one explanatory line and a single "Back to KronoSync" button; deliberately no in-overlay way to cancel the session (that defeats the commitment-device purpose). The only way to end early is `LockInSheet`'s active-state view, which has an "End session early" text button — reachable only after the user has already returned to KronoSync, so ending isn't a one-tap bail from wherever they were, but isn't a hard dead-end either (a genuine emergency isn't `Force Stop`-only).
   - **F1** (lock-in toggle at task creation): added to `AddOrCopySheet`'s New-task form next to the date/time chips, persisted as `ScheduleBlock.lockIn` (Room v3 → v4). Also exposed in `EditBlockSheet` for consistency (not explicitly asked, but a natural extension of an existing edit surface).
   - **F2** (persistent control): a lock icon added to the right of the Now/List/Grid segmented row in `ScheduleScreen`, with a small dot badge when a session is active/counting down/on break. Opens `LockInSheet`, which renders differently per phase: idle → explainer + ad-hoc duration picker (capped 2h, `AdHocDurationOptions`) + permission-request buttons if Usage Access/overlay aren't granted yet; countdown → the F5 cancel button (mirrors the notification, for if the user opens the app during the countdown); active → remaining time, "Take a break" (only when `BreakPolicy` says available), "End session early"; on break → status only.
   - **F6** (analytics): `AnalyticsAggregator.lockInSummary()` sums completed (non-cancelled) sessions' `plannedMinutes` between a date range, split ad-hoc vs. task-linked per the earlier open question. Shown as a "Time in Lock-In" card on Day/Week/Month (unlike the activity pie/bar chart, included on Day too since a single day's lock-in minutes are still meaningful on their own).
   - **New permissions**: `PACKAGE_USAGE_STATS` (declared with `tools:ignore="ProtectedPermissions"`, gated by `AppOpsManager`/Settings in practice, not a manifest grant), `SYSTEM_ALERT_WINDOW`, `FOREGROUND_SERVICE`, `FOREGROUND_SERVICE_SPECIAL_USE`. Both Usage Access and overlay are "special access" permissions the user grants manually in Settings — `domain/lockin/LockInPermissions.kt` has the checks and the `Settings.ACTION_*` intents to get there.
   - **Known limitations, not addressed this pass**: (1) if the service/process is killed by the OS under memory pressure mid-session, the `LockInSession` row's `endedAt` stays null forever (no watchdog/WorkManager reconciliation built) — analytics would simply not count that session since `getCompletedBetween` requires `endedAt IS NOT NULL AND cancelled = 0`, so it fails safe (undercounts) rather than corrupting data, but the row is never cleaned up either; (2) genuinely untested end-to-end on a real device this session — see below.
   - **Verification status**: `./gradlew testDebugUnitTest assembleDebug` passes and the APK installs cleanly (confirmed once before the device dropped off wireless ADB mid-session). **Not yet verified live**: actually granting Usage Access + overlay permissions and confirming the block screen appears over a real disallowed app, the countdown/cancel flow, break timing, and the task-linked auto-trigger at a real block's start time. This needs a real pass before considering the feature done — reconnect the device to continue.
